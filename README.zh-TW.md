...
VideoClipper 是一款輕量級工具，讓你在使用 PotPlayer 觀看影片時標記片段，並透過簡單的 GUI 介面檢視與匯出所有剪輯片段。  
它能在不中斷播放、也不需要手動記錄時間戳的情況下，順暢完成「觀看 → 標記 → 匯出」的工作流程。

---

## Overview

VideoClipper 由兩個主要元件組成：
...
每一組 `start` 與 `end` 代表一個剪輯片段。

### 2. gui_app.exe

完成觀看後，你可以開啟 GUI 介面，載入影片檔與其對應的 `.marks` 檔案。  
GUI 會顯示所有偵測到的片段，並提供滑桿與按鈕進行細部調整，最後可一次匯出所有剪輯片段。

如果 **找不到** 對應的 `.marks` 檔案，GUI 會切換為手動模式（Manual Mode），允許完全以手動方式建立與調整剪輯片段。
...
使用者不需要安裝 Python 或任何開發相關的相依套件。

只需下載最新的 Release 套件，並依照下方說明使用即可。
...
- 在 PotPlayer 中設定滑鼠中鍵：
  1. 開啟 PotPlayer，按下 `F5` 進入 **偏好設定**。
  2. 前往 **一般 → 鍵盤**。
  3. 點擊 **新增** 以建立新的熱鍵。
  4. 點擊快捷鍵輸入欄位，然後按下 **滑鼠中鍵** 進行指派。
  5. 在動作選單中選擇：**其他 → 複製目前播放時間到剪貼簿**。
...
- 當你想標記某個片段時：
  ...
  - 在片段開始前稍微按下滑鼠中鍵 → 記錄一個 `start`
  - 在片段結束後稍微按下滑鼠中鍵 → 記錄一個 `end`
...
- 點擊 GUI 右下角的 **Add Clip** 按鈕
- GUI 會提示你確認新增剪輯片段
- 新增後的剪輯片段可用與其他片段相同的方式進行調整
...
解壓後，請將 `ffmpeg.exe` 複製到 repository 中以下路徑：
...
- background marker 只會在 PotPlayer 為前景視窗時記錄時間戳。
- 標記時間點後無法復原；每次按下滑鼠中鍵都會永久寫入。
- 不支援巢狀片段（一個片段包含另一個片段）。
